...
struct v2f {
    ...
    float4 FoamUV	: TEXCOORD9;
    #endif
    #endif

    #if _BUMPLARGE_ON
    float2 AnimUV2 	: TEXCOORD10;
    #endif
};
...
fixed4 frag( v2f i ) : SV_Target
{
	...
	half3 finalBump 	= UnpackNormalBlend(n1,n2,n3, _BumpStrength,_BumpLargeStrength);
	#else
	...
	//Depth	
	refraction.a 		= saturate((abs(DistZ - DepthUV.z) - _DepthEnd) / (_DepthStart - _DepthEnd));
	//Clean Depth
	cleanRefraction.a 	= saturate((abs(sceneZ - i.DepthUV.z) - _DepthEnd) / (_DepthStart - _DepthEnd));
	...
	#ifdef UNITY_PASS_FORWARDBASE
	#if _SCATTER_ON
	finalColor += finalColor * 2 * saturate(i.ambient.a) * (DiffuseTerm(worldN, lightDir) * 0.5 + 0.5);
	#endif
	#endif
	...
	//end CULL_FRONT
		...
		//WHITE CAPS ======================================================================
			...
			#if _DISPLACEMENTMODE_WAVE || _DISPLACEMENTMODE_GERSTNER
